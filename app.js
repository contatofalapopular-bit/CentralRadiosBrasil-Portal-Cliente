(() => {
  ...
  function init() {
    ...
    $$("[data-editor-page]").forEach(button => button.addEventListener("click", () => switchEditor(button.dataset.editorPage)));
  ...
  function forceLogout(message="") { state.token=""; state.dashboard=null; state.site=null; state.versions=[]; sessionStorage.removeItem(CONFIG.TOKEN_KEY); $("#app-view").classList.add("hidden"); $("#login-view").classList.remove("hidden"); if(message)showMessage("#login-message",message,"error"); }
  ...
  function renderDashboard(){
    ...
    text("#session-name",client.nome_radio||client.nome||"Cliente"); text("#welcome-title",client.nome_radio||"Portal do Cliente"); text("#welcome-subtitle",[client.cidade,client.estado].filter(Boolean).join(" — ")||"Central Rádios Brasil");
  ...
  function renderSite(){
    ...
    text("#manager-site-name",site.nome_site||"Meu site"); text("#manager-site-status",statusLabel(site.status_publicacao||site.status||"rascunho")); text("#technical-stream",site.stream_url||"Será definido pela Central.");
  ...
  function switchTab(tab){$$('[data-tab]').forEach(button=>button.classList.toggle('active',button.dataset.tab===tab));$$('[data-panel]').forEach(panel=>panel.classList.toggle('hidden',panel.dataset.panel!==tab));window.scrollTo({top:0,behavior:'smooth'});if(tab==="site")switchEditor(state.activeEditor);}
